...
def index():
    search_query = request.args.get('search')  # use request.args.get for GET requests
    ...
    if search_query:
        ...
        for video in video_items:
            ...
            video['views_to_subscribers_ratio'] = views / subscribers if subscribers != 0 else 'N/A'
            
            # Get the publish date and duration
            video['publishDate'] = video['snippet'].get('publishedAt', 'N/A')
            video['duration'] = video['contentDetails'].get('duration', 'N/A')
