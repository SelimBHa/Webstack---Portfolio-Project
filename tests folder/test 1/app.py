...
API_KEY = 'AIzaSyBEnGYinhrj9vYg3CIDqGMc7CWS_Ef13jc'
...
def fetch_videos(query, page_token=None):
    ...
    print(f"API response: {response.json()}")  # Log the API response for debugging
    return response.json()
...
def index():
    ...
    if request.method == 'POST':  # Only process form if it's a POST request
        ...
        print(f"Search query: {search_query}")  # Log the search query for debugging
        if search_query:
            data = fetch_videos(search_query)
            video_items = data.get('items', [])
            print(f"Video items: {video_items}")  # Log the fetched video items
